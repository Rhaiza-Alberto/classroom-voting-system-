...
// Get user information for better error messages
$userQuery = "SELECT full_name, student_id FROM users WHERE id = ?";
...
if (!$currentPositionId) {
    ...
    $candidatesQuery = "SELECT c.id, c.user_id, c.status, u.first_name, u.middle_name, u.last_name, u.student_id 
                   FROM candidates c 
...
$candidateInfoQuery = "SELECT u.full_name, u.student_id FROM candidates c JOIN users u ON c.user_id = u.id WHERE c.id = ?";
$infoStmt = $conn->prepare($candidateInfoQuery);
$infoStmt->bind_param("i", $candidateId);
$infoStmt->execute();
$candidateInfo = $infoStmt->get_result()->fetch_assoc();
$infoStmt->close();

// Store vote with candidate info
$voteStmt = $conn->prepare("INSERT INTO votes (session_id, voter_id, candidate_id, position_id, candidate_name, candidate_student_id, voted_at) VALUES (?, ?, ?, ?, ?, ?, NOW())");
$voteStmt->bind_param("iiiiss", $sessionId, $userId, $candidateId, $positionId, $candidateInfo['full_name'], $candidateInfo['student_id']);
                
                if ($voteStmt->execute()) {
                    $message = ' Your vote has been recorded successfully!';
                    $messageType = 'success';
                    $hasVoted = true;
                    $debugInfo[] = "SUCCESS: Vote recorded!";
